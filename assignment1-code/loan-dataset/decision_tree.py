...
This file implements the decision tree algorithm on the dataset 1.
@author: vmarois
"""
import pandas as pd
import sys
...
def perform_grid_search():
    ...
    # plt.show()
    plt.savefig('loan-dataset-plots/grid_search-dtree.png', bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    cross_validation_dist_plot(estimator=DecisionTreeClassifier(), title="dtree", location="loan-dataset-plots",
                               X=X_train, y=y_train, cv=10)

    perform_grid_search()

    plot_learning_curve(DecisionTreeClassifier(criterion='entropy'), "dtree", "loan-dataset-plots", X, y,
                        ylim=(0.4, 1.01), cv=StratifiedKFold(n_splits=10, random_state=42), n_jobs=4,
                        train_sizes=np.linspace(.1, 1.0, 5))

    plot_validation_curve(estimator=DecisionTreeClassifier(criterion='entropy'), title="dtree",
                          location="loan-dataset-plots", xlabel="max_depth", ylabel="Score", X=X, y=y,
                          param_name="max_depth", ylim=(0.4, 1.01), cv=StratifiedKFold(n_splits=10, random_state=42),
